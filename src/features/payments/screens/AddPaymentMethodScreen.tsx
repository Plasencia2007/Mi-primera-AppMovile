    ...
    <View style={[
      ...
      {showIcon && IconRight && (
        <View style={!editable && { opacity: 0.5 }}>
          <IconRight size={20} color={error ? "#EF4444" : (!editable ? "#94A3B8" : "#FF6347")} />
        </View>
      )}
    </View>
...
export const AddPaymentMethodScreen = ({ onBack, paymentToEdit }: AddPaymentMethodScreenProps) => {
  ...
  const [activeTab, setActiveTab] = useState<'TARJETA' | 'YAPE' | 'PLIN'>(() => {
    ...
    if (paymentToEdit.type === 'VISA' || paymentToEdit.type === 'MASTERCARD') return 'TARJETA';
  ...
  const form = useForm<PaymentFormData>({
    ...
    defaultValues: {
      ...
      cardNumber: paymentToEdit?.details?.lastFour ? `0000 0000 0000 ${paymentToEdit.details.lastFour}` : '',
  ...
  const { control, handleSubmit, watch, setError, formState: { errors } } = form;
  ...
  const onValidSubmit = (data: PaymentFormData) => {
    if (activeTab === 'TARJETA') {
      ...
      if (isEditing) {
        updateMethod(paymentToEdit.id, {
          type,
          title: `•••• ${lastFour}`,
          subtitle: `${type === 'VISA' ? 'Débito' : 'Crédito'} • Vence ${data.expiry}`,
          details: {
      ...
      if (!data.cardHolder || data.cardHolder.trim().length < 3) {
        setError('cardHolder', { message: 'Ingrese el nombre del titular (mín. 3 caracteres)' });
        return;
      }
      
      const cleanPhone = (data.phoneNumber || '').replace(/\D/g, '');
      if (cleanPhone.length !== 9 || !cleanPhone.startsWith('9')) {
        setError('phoneNumber', { message: 'El número debe ser de 9 dígitos y empezar con 9' });
        return;
      }
      ...
      if (isEditing) {
        updateMethod(paymentToEdit.id, updatedMethodData);
      } else {
        addMethod({
          id: Math.random().toString(36).substr(2, 9),
          ...updatedMethodData,
    ...
    // Success! Show modal with small delay for better UX
    setTimeout(() => {
    ...
    >
      <View style={styles.header}>
        ...
        <Text style={styles.headerTitle}>
          {isEditing ? 'EDITAR' : 'AGREGAR'} <Text style={{ color: colors.primary }}>MÉTODO</Text>
        </Text>
        <View style={{ width: 44 }} />
      </View>
          ...
          <Controller
            ...
            render={({ field: { onChange, onBlur, value }, fieldState: { error } }) => (
              <ControlledInputField
                ...
                onBlur={onBlur}
                placeholder={activeTab === 'TARJETA' ? "Nombre como figura en la tarjeta" : "Nombre del titular de la cuenta"}
                error={error?.message}
              />
            )}
          />
              ...
              <Controller
                ...
                render={({ field: { onChange, onBlur, value }, fieldState: { error } }) => (
                  <ControlledInputField
                    ...
                    value={isEditing ? `•••• •••• •••• ${paymentToEdit.details.lastFour}` : formatCardNumber(value || '')}
          ...
          >
            <View style={styles.cardPreviewContent}>
              <View style={styles.cardTop}>
                ...
                <Rss size={24} color="rgba(255,255,255,0.8)" style={styles.contactlessIcon} />
              </View>
              ...
              <View style={styles.previewFooter}>
                <View style={{ flex: 1 }}>
                  <Text style={styles.previewValue}>{currentCardHolder.toUpperCase() || 'NOMBRE DEL TITULAR'}</Text>
                </View>
                <View style={{ alignItems: 'flex-end' }}>
                  <Text style={styles.previewLabel}>VENCE</Text>
                  <Text style={styles.previewValue}>{formatExpiry(currentExpiry) || '00/00'}</Text>
                </View>
              </View>
